...
def send_telegram_alert(temp_object, temp_air, humidity, pressure):
    ...
    msg = (
        "⚠️ *Solar Panel Alert*\n\n"
        f"🔥 Surface Heat: *{temp_object:.2f} °C*\n"
...
<div class="tcam"><div class="tcam-img">📷</div><div><div class="tcam-h">TC001 Thermal Camera <span class="tcam-status"><span style="width:6px;height:6px;border-radius:50%;background:#fb923c"></span>Pending Hardware</span></div><div class="tcam-p">High-resolution thermal imaging will stream here once the TC001 camera is connected via Raspberry Pi. Live video, hotspot detection overlays, and recording controls will replace this placeholder.</div><div class="tcam-list"><span class="tcam-tag">256×192 Resolution</span><span class="tcam-tag">-20°C → 550°C Range</span><span class="tcam-tag">25Hz Refresh</span><span class="tcam-tag">USB-C → Raspberry Pi</span></div></div></div>
...
def report_pdf():
    ...
    sensor_data = [
        ...
        ["Surface Heat",     f"{latest_data.get('temp_object', 0):.2f}",  "°C"],
        ["Ambient (IR)",     f"{latest_data.get('temp_ambient', 0):.2f}", "°C"],
